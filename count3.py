        ...
        for op_combinations in product(ops, repeat=3):
            try:
                ...
                if result == target:
                    ...
                    # 組み合わせをソートして保存
                    combinations.add(''.join(sorted(combination_str)))
...
for i, combination_str in enumerate(combinations, 1):
    # 数値と演算子を交互に表示
    formatted_combination = ''.join(combination_str[i] for i in range(len(combination_str)) if i % 2 == 0)
    formatted_combination += ''.join(combination_str[i] for i in range(len(combination_str)) if i % 2 != 0)
    print(f"組み合わせ{i}: {formatted_combination}")